...
def BuildImageManifestRows(DataRootDirectory: Path) -> tuple[list[dict[str, object]], dict[str, object]]:
    ...
    _, Mri3MetaIndex, Mri3MetaSourceFiles = LoadAndIndexFirstTable(Mri3MetaCsvFiles)
    _, MriQcIndex, MriQcSourceFiles = LoadAndIndexFirstTable(MriQcCsvFiles)
    _, MriMprageRankIndex, MriMprageRankSourceFiles = LoadAndIndexFirstTable(MriMprageRankCsvFiles)
    ...
    for NiftiRow in NiftiRows:
        ImageIdKey = str(NiftiRow["ImageIdKey"])

        IdaMetadataRow = GetFirstIndexedRow(ImageIdKey, IdaMetadataIndex)
        Mri3MetaRow = GetFirstIndexedRow(ImageIdKey, Mri3MetaIndex)
        MriQcRow = GetFirstIndexedRow(ImageIdKey, MriQcIndex)
        MriMprageRankRow = GetFirstIndexedRow(ImageIdKey, MriMprageRankIndex)
        ...
        ImageManifestRows.append(
            {
                ...
                "FieldStrength": GetMetadataValue(
                    PrimaryRow=Mri3MetaRow,
                    SecondaryRow=IdaMetadataRow,
                    ExactColumnCandidates=["FieldStrength", "Field Strength", "MagStrength", "MAGSTRENGTH", "Tesla"],
                ...
                "Manufacturer": GetMetadataValue(
                    PrimaryRow=Mri3MetaRow,
                    SecondaryRow=IdaMetadataRow,
                    ExactColumnCandidates=["Manufacturer", "Mfg", "MFG"],
                    FragmentColumnCandidates=["Manufacturer", "MFG"],
                ),
                "ScannerModel": GetMetadataValue(
                    PrimaryRow=Mri3MetaRow,
                    SecondaryRow=IdaMetadataRow,
                    ExactColumnCandidates=["MfgModel", "Mfg Model", "Model", "ScannerModel", "Scanner Model"],
                    FragmentColumnCandidates=["Model"],
                ),
                "QcStatus": GetFirstAvailableValueContaining(
                    MriQcRow,
                    ["QC", "Quality", "Pass", "Fail", "Status"],
                ),
                ...
                "Mri3MetaMatchCount": CountIndexedRows(ImageIdKey, Mri3MetaIndex),
                "MriQcMatchCount": CountIndexedRows(ImageIdKey, MriQcIndex),
                "MprageRankMatchCount": CountIndexedRows(ImageIdKey, MriMprageRankIndex),
...
def Main() -> None:
    ...
    print("Image manifest build complete.")
